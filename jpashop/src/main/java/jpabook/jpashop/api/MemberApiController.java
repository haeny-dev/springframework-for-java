...
public class MemberApiController {
    ...
    /**
     * 회원 조회 API V1: 응답 값으로 엔티티를 직접 외부에 노출한다.
     *
     * 문제점
     * - 엔티티에 프레젠테이션 계층을 위한 로직이 추가된다.
     * - 기본적으로 엔티티의 모든 값이 노출된다.
     * - 응답 스펙을 맞추기 위해 로직이 추가된다.
     * - 실무에서는 같은 엔티티에 대해 API 가 용도가 다양하게 만들어지는데,
    ...
    /**
     * 회원 등록 API V1: 요청 값으로 Member 엔티티를 직접 받는다.
     * <p>
     * 문제점
     * - 엔티티에 프레젠테이션 계층을 위한 로직이 추가된다.
    ...
    /**
     * 회원 등록 API V2: 요청 값으로 Member 엔티티 대신에 별도의 DTO 를 받는다.
     *
     * @param request
     * @return
     */
